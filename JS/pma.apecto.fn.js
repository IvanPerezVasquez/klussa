...
function aspectos(){
  ...
  $.ajax({
    url: '../DATABASE/cargar_aspectos.php',
    type: 'POST',
      ...
      if(!json.err){
        ...
        if(i!="err"){
          ...
          <td >`+item.plan_nom+`</td>
            ...
            <td id ="`+item.PK_asp_amb+`" plan ="`+item.plan_nom+`" sub = "`+item.sub_plan+`"  asp = "`+item.aspecto_amb+`" >
...
title_modal_up = 'Insertar Aspecto Ambiental';
...
<div class="form-group">
 ...
 <div class="form-group">
            <label for="ci" class="col-form-label"><b> ASPECTO AMBIENTAL:</b></label>
            <input type="text" class="form-control" name="asp" id="asp" >
          </div>
...
if (cbx === "") {
  showToastError("Debe selecionar un subplan.");
  return;
}

if (asp === "") {
  showToastError("Debe ingresar un aspecto.");
  return;
}
...
function inser_reg (){
  ...
  $.ajax({
    type:"POST",
    url:"../DATABASE/insert_aspecto.php",
    data:parametros,
    contentType:false,
    processData: false, 
    success:function(response){
...
<div class="form-group">
 ...
 <div class="form-group">
          ...
          <textarea class="form-control" name="asp" id="asp">`+ asp +`</textarea>
          </div>
...
/////////////////////////////////////////////////
  ...
  $.ajax({
    url: '../DATABASE/up_history_aspecto.php',
    type: 'POST',
    data:{ id},
    success: function(response){
      console.log(response);
                        ...
                        <div class="row no-gutters">
                          ...
                          <div class="col-md-8">
                            <div class="card-body">
                               ...
                               <h5> <strong>Usuario Modificaciones: </strong>`+item.usr_up_aut_asp+`  </h5>
                              <p class="card-text"><strong> Fecha Registro: </strong> `+item.fc_reg_aut_asp+`</p>
                               <p class="card-text"><strong> Fecha Actualizacion: </strong> `+item.fc_up_aut_asp+`</p>
